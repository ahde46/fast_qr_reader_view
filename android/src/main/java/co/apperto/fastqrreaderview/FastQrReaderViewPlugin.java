...
public class FastQrReaderViewPlugin implements MethodCallHandler {
    ...
    // Whether we should ignore process(). This is usually caused by feeding input
    // data faster than
    ...
    void startScanning(@NonNull Result result) {
        ...
        //
        // FirebaseVisionImage test =
        // FirebaseVisionImage.fromByteBuffer(image.getPlanes()[0].getBuffer(), new
        // FirebaseVisionImageMetadata.Builder()
        // .setFormat(FirebaseVisionImageMetadata.IMAGE_FORMAT_NV21)
        // .setHeight(image.getHeight())
        // .setWidth(image.getWidth())
        // .build());
        //// FirebaseVisionImage test = FirebaseVisionImage.fromMediaImage(image,
        // FirebaseVisionImageMetadata.ROTATION_0); // Slower in my experience
        // image.close();
        //
        // camera.codeDetector.detectInImage(test).addOnSuccessListener(new
        ...
        // channel.invokeMethod("updateCode",
        // firebaseVisionBarcodes.get(0).getRawValue());
    ...
    private class QrReader {
        ...
        //
        QrReader(final String cameraName, final String resolutionPreset, final ArrayList<String> formats,
                @NonNull final Result result) {

            // AVAILABLE FORMATS:
            // enum CodeFormat { codabar, code39, code93, code128, ean8, ean13, itf, upca,
            // upce, aztec, datamatrix, pdf417, qr }
        ...
        private void registerEventChannel() {
            new EventChannel(registrar.messenger(), "fast_qr_reader_view/cameraEvents" + textureEntry.id())
        ...
        //
        private void computeBestPreviewAndRecordingSize(StreamConfigurationMap streamConfigurationMap,
                Size minPreviewSize, Size captureSize) {
            Size[] sizes = streamConfigurationMap.getOutputSizes(SurfaceTexture.class);
            float captureSizeRatio = (float) captureSize.getWidth() / captureSize.getHeight();
            List<Size> goodEnough = new ArrayList<>();
            for (Size s : sizes) {
                if ((float) s.getWidth() / s.getHeight() == captureSizeRatio && minPreviewSize.getWidth() < s.getWidth()
            ...
            if (goodEnough.isEmpty()) {
                ...
                // Video capture size should not be greater than 1080 because MediaRecorder
                // cannot handle higher resolutions.
        ...
        private void computeBestCaptureSize(StreamConfigurationMap streamConfigurationMap) {
            ...
            captureSize = Collections.max(Arrays.asList(streamConfigurationMap.getOutputSizes(ImageFormat.YUV_420_888)),
        ...
        // cameraCaptureSession.setRepeatingRequest(captureRequestBuilder.build(), new
